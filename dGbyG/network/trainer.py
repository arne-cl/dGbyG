...
class Model(object):
    ...
    def train(self, dataset:Train_Dataset, epochs:int, lr:float, weight_decay=0) -> tuple:
        # train
    ...
    def __train(self, network:nn.Module, data, S, dgs, weight, epochs, lr, weight_decay=0):
        ...
        min_loss = torch.inf
        for epoch_counter in tqdm(iterable=range(epochs), desc="training"):
            optimizer.zero_grad()
    ...
    def __cross_validation(self, network, data, train_S, train_dGs, val_S, epochs, lr, train_weight, weight_decay=0, desc=''):
        # 
        # 
        ...
        for epoch_counter in trange(epochs, desc=desc, leave=True):
            ...
            cids_energy = network(data).view(1, -1)
            p_dGs = torch.mm(cids_energy, train_S).view(-1)
            ...
            network.eval()
            with torch.no_grad():
                #assert torch.allclose(cids_energy, model(data).view(1, -1))
                val_dGs = torch.mm(cids_energy, val_S).view(-1, 1)
